        ...
        with psycopg2.connect(database=self.database, user=self.login, password=self.password) as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    ret = f'Made {int(output[0]) + int(output[1])} tries, {output[0]} correct answers and {output[1]} wrong answers. {round(int(output[0])/(int(output[0]) + int(output[1])),2) * 100}% of correct answers!🔥 Keep going! 🚀'
                    return ret
                except Exception as ex:
                    template = "An exception of type {0} occurred. Arguments:\n{1!r}"
                    message = template.format(type(ex).__name__, ex.args)
